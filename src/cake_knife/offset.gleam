//// Offset-based pagination for Cake SQL queries.
////
//// This module provides traditional page-based pagination using LIMIT/OFFSET.
//// Use `limit()`, `offset()`, `page()`, and `paginate()` for page-based navigation,
//// and the `Page(a)` type for working with paginated results and metadata.
...
/// |> select.from_table("users")
/// |> select.to_query
/// |> offset.paginate(page: 2, per_page: 10, max_per_page: 100)
/// // -> Ok(query with LIMIT 10 OFFSET 10)
///
/// select.new()
/// |> select.from_table("users")
/// |> select.to_query
/// |> offset.paginate(page: 0, per_page: 10, max_per_page: 100)
/// // -> Error(InvalidPage(0))
/// ```
pub fn paginate(
  query qry: ReadQuery,
