...
import gleeunit
import test_helper/shork_test_helper
...
pub fn paginate_with_valid_params_test() {
  ...
  let assert Ok(results) = shork_test_helper.setup_and_run(paginated_query)
...
pub fn combined_query_union_ignores_limit_integration_test() {
  ...
  let assert Ok(results) = shork_test_helper.setup_and_run(combined_query)
...
pub fn keyset_pagination_complete_workflow_test() {
  ...
  let assert Ok(results1) = shork_test_helper.setup_and_run(query1)
  ...
  let assert Ok(results2) = shork_test_helper.setup_and_run(query2)
...
pub fn keyset_pagination_end_to_end_scenario_test() {
  ...
  let assert Ok(first_results) = shork_test_helper.setup_and_run(first_query)
  ...
  let assert Ok(second_results) = shork_test_helper.setup_and_run(second_query)
